...
def Login():
    driver.find_element(by=By.XPATH, value="//*[@class='btn-signin item d-none d-lg-inline-block']").click()
    sleep(1)
    username = driver.find_element(By.ID, 'username')
    username.send_keys(email)
    pwd = driver.find_element(By.ID, "password")
    pwd.send_keys(password)
    driver.find_element(by=By.XPATH, value="//button[@class='btn-red btn-signin']").click()
    sleep(2)
    driver.implicitly_wait(50)
    cur_url=driver.current_url
    while cur_url == base_url:
        driver.find_element(by=By.PARTIAL_LINK_TEXT, value=f"{concert}").click()
        driver.implicitly_wait(30)
        cur_url=driver.current_url
    driver.implicitly_wait(30)

def SelectShow():
    row=len(driver.find_elements(by=By.XPATH, value="//div[@class='box-event-list']/div[2]/div"))
    #select round
    driver.find_element(by=By.XPATH, value=f"//div[@class='box-event-list']/div[2]/div[{show}]/div[2]/span[1]/a[1]").click()
    driver.implicitly_wait(30)
        

    selected=driver.find_element(by=By.XPATH, value="//*[@id='rdId']/option[1]").text
